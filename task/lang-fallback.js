exports.test = function(cfg) {
  ...
  if(lf.length<3 || lf[0]!='in' || lf[1]!='layout.text-field' || lf.some(function(item) {
        return typeof item != 'string';
      })) {
    throw new Error('lang-fallback.layer-filter: expected filter ["in", "layout.text-field", "value1", ...]');
  }
  ...
  decs.forEach(function(dec, idx) {
    ...
    if(typeof postfix != 'string') {
      throw new Error('lang-fallback.decorators[].layer-name-postfix: expected string, null, or undefined');
    }
    decs.slice(idx+1).forEach(function(dec2) {
      var postfix2 = dec2['layer-name-postfix'] || '';
      if(postfix == postfix2) {
        throw new Error('lang-fallback.decorators[].layer-name-postfix: expected unique values: '+[postfix, postfix2].join(', '));
      }
    });

    var tf = dec['layout.text-field'];
    if(typeof tf != 'string') {
      throw new Error('lang-fallback.decorators[].layout.text-field: expected string');
    }
    var fap = dec['filter-all-part'];
    if(!(fap instanceof Array)) {
      throw new Error('lang-fallback.decorators[].filter-all-part: expected Array');
    }
  });
};
